...
#
# Modified by Chiara Ruggeri (support@meteca.org)
#
...
MD5_TIMEOUT_PER_MB = 10               # timeout (per megabyte) for calculating md5sum
...
class ESPLoader(object):
    ...
    def __init__(self, port=DEFAULT_PORT, baud=ESP_ROM_BAUD, trace_enabled=False):
        ...
        # send commands to sam-ba bootloader
        self._port.write('N#')
        ch = self._port.readline()
        self._port.write('L#')
        self._port.write('@#')
...
def write_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        ...
        t = time.time()
        while len(image) > 0:
            print('Writing at 0x%08x... (%d %%)' % (address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks))
            sys.stdout.flush()
