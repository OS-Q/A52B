...
const uint32_t defaultBaud = 115200;
...
void Uart::begin(unsigned long baudrate, uint16_t config)
{
  if (baudrate == AUTOBAUD){
    if (uc_pinRX == PIN_SERIAL2_RX){ // Identifies the serial port connected to the ESP32
      // Ask our companion for current baud setting on hardware serial port, and set SerialCompanion baudrate to same
	  #ifdef COMMUNICATION_LIB_ENABLED
		baudrate = sendSystemGet(SYSTEMCMD_GET_BAUD);
	  #else
		baudrate = defaultBaud;
	  #endif
    }
    // if we are on our controller, set to defaultBaud
    else // if (uc_pinRX == PIN_SERIAL1_RX)
    {
      baudrate = defaultBaud;
    }
  }
