...
static voidFuncPtrArg ISRcallback[EXTERNAL_NUM_INTERRUPTS];
static void* ISRargs[EXTERNAL_NUM_INTERRUPTS];
 ...
 */
void enableInterrupt(uint32_t pin, voidFuncPtrArg callback, void* arg, uint32_t mode)
{

  // Handle attachInterrupt on pins connected to communication chip
  if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
	  #ifdef COMMUNICATION_LIB_ENABLED
	  sendAttachInt(SET_INTERRUPT, pin, callback, arg, mode);
	  #endif
	  return;
  }
  ...
  if (callback)
  {
	...
	ISRargs[current] = arg; // List of agruments
...
{
	enableInterrupt(pin, (voidFuncPtrArg)callback, NULL, mode);
}

void attachInterruptArg(uint32_t pin, voidFuncPtrArg callback, void* args, uint32_t mode)
{
	enableInterrupt(pin, callback, args, mode);
}
...
void detachInterrupt(uint32_t pin)
{
  ...
  for (; current<nints-1; current++) {
	...
	ISRargs[current] = ISRargs[current+1];
  }
  nints--;
}
...
void EIC_Handler(void)
{
  ...
  for (uint32_t i=0; i<nints; i++)
  {
    if ((EIC->INTFLAG.reg & ISRlist[i]) != 0)
    {
	  ...
	  if(ISRargs[i] != NULL)
		  ISRcallback[i](ISRargs[i]);
	  else
		((voidFuncPtr)ISRcallback[i])();
      // Clear the interrupt
