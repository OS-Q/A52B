...
void USBDeviceClass::disable(){
	detach();
	end();
	standby();
}

bool USBDeviceClass::isEnabled(){
	return enabled;
}
...
void USBDeviceClass::initEndpoints() {
	for (uint8_t i = 1; (i < EP_ARRAY_SIZE) && (EndPoints[i] != 0); i++) {
		initEP(i, EndPoints[i], EPAddr[i]);
	}
}

void USBDeviceClass::initEP(uint32_t ep, uint32_t config, uint8_t addr)
{
	if (config == (USB_ENDPOINT_TYPE_INTERRUPT | USB_ENDPOINT_IN(addr)))
	{
		usbd.epBank1SetSize(ep, 64);
		usbd.epBank1SetAddress(ep, &udd_ep_in_cache_buffer[ep]);
		usbd.epBank1SetType(ep, 4); // INTERRUPT IN
	}
	else if (config == (USB_ENDPOINT_TYPE_BULK | USB_ENDPOINT_OUT(addr)))
	{
