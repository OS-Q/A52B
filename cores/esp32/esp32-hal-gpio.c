...
extern void IRAM_ATTR __pinMode(uint8_t pin, uint8_t mode)
{
		...
		pin = g_APinDescription[pin].ulPin;
        if(pin == 21)
...
extern void IRAM_ATTR __digitalWrite(uint8_t pin, uint8_t val)
{
		...
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
				sendGPIOWrite(WRITE_DIGITAL, pin, val);
			#endif // COMMUNICATION_LIB_ENABLED
			return;
		}

		if(pin == COMPANION_RESET)
			if(val){
				// set pin as INPUT_PULLUP instead of OUTPUT HIGH
...
extern void IRAM_ATTR __digitalToggle(uint8_t pin){
	#ifdef BRIKI_MBC_WB_ESP
		if(pin > PINS_COUNT) {
			return;
		}
		...
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
				sendGPIOWrite(WRITE_TOGGLE, pin);
			#endif // COMMUNICATION_LIB_ENABLED
			return;
		}

		if(pin == COMPANION_RESET)
			// Do not toggle reset - use digitalWrite instead
			return;
	...
	if(pin < 32){
		uint32_t value = GPIO.out & ((uint32_t)1 << pin);
		value = !value;
		if(value)
			GPIO.out_w1ts = ((uint32_t)1 << pin);
		else
			GPIO.out_w1tc = ((uint32_t)1 << pin);
		//GPIO.out = ((uint32_t)value << pin);
	}
	else if(pin < 34) {
		uint32_t value = GPIO.out1.val & ((uint32_t)1 << (pin - 32));
		value = !value;
		if(value)
			GPIO.out1_w1ts.val = ((uint32_t)1 << (pin - 32));
		else
			GPIO.out1_w1tc.val = ((uint32_t)1 << (pin - 32));
		//GPIO.out1.val = ((uint32_t)value << (pin - 32));
    }
}
...
extern int IRAM_ATTR __digitalRead(uint8_t pin)
{
		...
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
				return sendGPIORead(READ_DIGITAL, pin);
			#else
				return 0;
			#endif // COMMUNICATION_LIB_ENABLED
			}
...
{
	#ifdef BRIKI_MBC_WB_ESP
		// Handle attachInterrupt on pins connected to control chip
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
				sendAttachInt(SET_INTERRUPT, pin, userFunc, arg, intr_type);
			#endif // COMMUNICATION_LIB_ENABLED
			return;
			}

		if(pin > PINS_COUNT || g_APinDescription[pin].ulPort == PIO_NOT_A_PORT) {
			return;
		}
		pin = g_APinDescription[pin].ulPin;
	#endif
...
{
	#ifdef BRIKI_MBC_WB_ESP
		// Handle detachInterrupt on pins connected to control chip
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
			sendDetachInt(UNSET_INTERRUPT, pin);
			#endif // COMMUNICATION_LIB_ENABLED
			return;
		}
...
extern void digitalToggle(uint8_t pin) __attribute__ ((weak, alias("__digitalToggle")));
