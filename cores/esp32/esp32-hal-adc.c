...
//static uint8_t __analogWidth = 1;//10 bits
static uint8_t __analogWidth = 3;//12 bits
static uint8_t __analogCycles = 8;
static uint8_t __analogSamples = 0;//1 sample
static uint8_t __analogClockDiv = 1;
static uint8_t __writeResolution = 8;
static bool __pwmInstance[40];

// Width of returned answer ()
//static uint8_t __analogReturnedWidth = 10;
...
bool IRAM_ATTR __adcAttachPin(uint8_t pin){
    ...
    } else if(g_APinDescription[pin].ulPin == 25){
        CLEAR_PERI_REG_MASK(RTC_IO_PAD_DAC1_REG, RTC_IO_PDAC1_XPD_DAC | RTC_IO_PDAC1_DAC_XPD_FORCE);//stop dac1
    } else if(g_APinDescription[pin].ulPin == 26){
...
uint16_t IRAM_ATTR __analogRead(uint8_t pin)
{
	...
	// Handle read on pins connected to control chip
	if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
		#ifdef COMMUNICATION_LIB_ENABLED
			return sendGPIORead(READ_ANALOG, pin);
		#else
			return 0;
...
int __hallRead()    //hall sensor without LNA
{
    ...
    Sens_Vp0 = __analogRead(43);
    Sens_Vn0 = __analogRead(44);
    SET_PERI_REG_MASK(RTC_IO_HALL_SENS_REG, RTC_IO_HALL_PHASE);
    Sens_Vp1 = __analogRead(43);
    Sens_Vn1 = __analogRead(44);
...
// Arduino-like analogWrite custom implementation. For better performances, use esp-provided functions.
extern void __analogWrite(uint8_t pin, uint8_t val){
	#ifdef BRIKI_MBC_WB_ESP
		if(pin > PINS_COUNT) {
			return;
		}
		
		// Handle write on pins connected to control chip
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
				sendGPIOWrite(WRITE_ANALOG, pin, val);
			#endif // COMMUNICATION_LIB_ENABLED
			return;
		}
	#endif
	...
	uint8_t channel = 0;
	if(!__pwmInstance[pin]){
		__pwmInstance[pin] = true;
		ledcSetup(channel, 732/*BASE_FREQ*/, __writeResolution);
...
extern void __analogWriteFreq(uint8_t pin, uint8_t val, uint32_t frequency){
	#ifdef BRIKI_MBC_WB_ESP
		if(pin > PINS_COUNT) {
			return;
		}
		...
		if(g_APinDescription[pin].ulPinType == PIO_EXTERNAL){
			#ifdef COMMUNICATION_LIB_ENABLED
				sendGPIOWrite(WRITE_ANALOG_F, pin, val, frequency);
			#endif // COMMUNICATION_LIB_ENABLED
			return;
		}
	#endif
	...
	uint8_t channel = 0;
	__pwmInstance[pin] = true;
	ledcSetup(channel, frequency, __writeResolution);
	ledcAttachPin(pin, channel);
...
extern void analogWrite(uint8_t pin, uint8_t val) __attribute__ ((weak, alias("__analogWrite")));
extern void analogWriteFreq(uint8_t pin, uint8_t val, uint32_t frequency) __attribute__ ((weak, alias("__analogWriteFreq")));
