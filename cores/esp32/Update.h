...
#error "Sorry, Update library doesn't work with samd21 processor"
#endif 
...
#define U_SPIFFS  100
#define U_AUTH    200
#define U_COMPANION    300

#define COMPANION_FLASH_ADDR 0x2000
#define COMPANION_RAM_ADDR 0x20005000
...
class UpdateClass {
  public:
    ...
    */
	...
	/*
	  send reset command to companion microcontroller to start the just updated application
	*/
	void resetCompanion();
  ...
  private:
    ...
    bool _verifyEnd();
	bool _flashCompanion();
	bool _initializeBootloader();
	bool _eraseCompanion();
	bool _checkCompanionReply(char expected);
	bool _writeInBlocks(uint8_t* buffer, uint16_t len);
