...
{
    gpio_matrix_out(g_APinDescription[pin].ulPin, function, invertOut, invertEnable);
}

void IRAM_ATTR pinMatrixOutDetach(uint8_t pin, bool invertOut, bool invertEnable)
{
    gpio_matrix_out(g_APinDescription[pin].ulPin, MATRIX_DETACH_OUT_SIG, invertOut, invertEnable);
}

void IRAM_ATTR pinMatrixInAttach(uint8_t pin, uint8_t signal, bool inverted)
{
    gpio_matrix_in(g_APinDescription[pin].ulPin, signal, inverted);
}
